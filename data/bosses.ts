...
export const BOSSES: Boss[] = [
  ...
  },
  {
    ...
    video: 'EL9eedriOmM',
    setup: { t: 2, h: 3, d: 5 },
    rule: 'Never let any Crawler Mines detonate freely. Assign your soakers before the pull - on Heroic each stomp hits for ~1.5M physical.',
  ...
  },
  {
    ...
    video: '',
    setup: { t: 2, h: 2, d: 6 },
    rule: 'Ancient Miasma replaces ALL healing with shields. Orbs instantly strip your shield - never touch them.',
    overview: 'Two alternating phases. Phase 1 (~2 minutes): Ancient Miasma is active - healers build shields. Deal with imploding circles that must be soaked, and smashes that must be avoided. After two cycles Malkorok reaches 100 Rage and enters Blood Rage. Phase 2 (~20 seconds): Miasma drops, healing works normally, both tanks stack in front to split the ~2.4M cleave, use heavy cooldowns. Phase 1 resumes with +25% more damage on the boss. Hard 6-minute enrage.',
    mechs: [
      { icon: '🛡️', name: 'Ancient Miasma', important: true, desc: 'All healing replaced by absorption shields for Phase 1. Miasma also ticks ~40k Shadow every 2 seconds, so shields drain even when nothing is hitting you.' },
      { icon: '🔵', name: 'Imploding Energy Soak', important: true, desc: 'Three circles spawn every ~20 seconds. Each must be soaked by exactly one player. If unsoaked it explodes for ~700k raid-wide. Never double up. Assign areas before the pull so everything is covered.' },
      { icon: '🌪️', name: 'Arcing Smash + Breath of Y\'Shaarj', important: true, desc: 'Malkorok does three Arcing Smash cones (~810k Physical), then the three cone areas re-explode for ~1.6M Shadow with no warning animation. Mark each smash location with a raid marker as it lands.' },
      { icon: '🌑', name: 'Essence of Y\'Shaarj Orbs (Heroic)', important: true, desc: 'Persistent orbs on the ground that instantly destroy your shield on contact - then deals 150k Shadow directly to health. The room fills over time.' },
    ...
    heroic: [
      ...
      'Displaced Energy now roots its targets in place. Dispel Displaced Energy immediately.',
    ...
    roles: [
      ...
      { color: '#1db89a', label: 'Healers', note: 'Phase 1: heal the raid to build up shields. Phase 2 transition: use a throughput cooldown in the 4-second grace window to top all shields after Miasma reapplies. Dispel Displaced Energy.' },
      { color: '#2e8fdf', label: 'DPS', note: 'Own a quadrant for Imploding Energy - use a personal defensive when soaking. Never touch orbs. During Blood Rage: immunity users clear orbs. Ranged stay spread enough that orbs don\'t block movement lanes.' },
    ],
    strat: [
      {
        phase: 'Phase 1 - Ancient Miasma',
        points: [
          { icon: null, role: 'All', note: 'Spread in pre-assigned quadrants. Use raid markers to track each Arcing Smash so the raid knows where Breath will re-explode. Keep moving slightly to avoid standing on orbs - but stay in quadrant to react to your Imploding Energy circle. After the third Arcing Smash, the three marked cone areas re-explode for ~1.6M Shadow damage with no warning animation. Find the gap between markers and stand there. If caught out of position, use a large personal defensive.' },
          { icon: '🛡️', role: 'Tanks', note: 'Swap around 10–15 Fatal Strike stacks - off-tank must have a shield before taunting. Immediately taunt Living Corruption adds to center on every Seismic Slam. Save major cooldowns for Blood Rage.' },
          { icon: '💚', role: 'Healers', note: 'Phase 1: heal the raid to build up shields. Priority: red-tier players and anyone about to soak a circle.' },
          { icon: '⚔️', role: 'DPS', note: 'Stay in your quadrant. Soak your Imploding Energy circle with a personal defensive. Sidestep Arcing Smashes and remember their locations. Keep moving to prevent orbs from boxing you in.' },
        ],
      },
      {
        phase: 'Blood Rage (Phase 2)',
        points: [
          { icon: null, role: 'All', note: 'Both tanks stack in front of the boss - each hit is 2.4M split among all players. Healing works normally. Use major damage reduction cooldowns to help tanks survive. After Blood Rage (~20 sec), 4-second grace window before Miasma reapplies - top actual health before shields take over.' },
          { icon: '🛡️', role: 'Tanks', note: 'Use all major defensive cooldowns and externals to survive the 20second phase.' },
          { icon: '💚', role: 'Healers', note: 'Dispel Displaced Energy as everyone should stay spread, on Heroic it roots, so don\'t wait for targets to run out.' },
          { icon: '⚔️', role: 'DPS', note: 'Immunity users clear orbs during this phase to reduce clutter in Phase 1.' },
        ],
      },
      {
  ...
  },
  {
    ...
    rule: 'Every crate your side clears sends an Unstable Spark to the other side. Our route: 2 buff + 1 boss + all 6 mediums + all 18 smalls = 50 energy. Never open the boss and mediums at the same time.',
    ...
    mechs: [
      ...
      { icon: '🪨', name: 'Stone Statues (Mogu Boss)', important: true, desc: 'The Elder Council boss spawns a stone statue on every cast. Each active statue increases pulsing AoE by 20% (stacks fast). Kill statues as they spawn - never let more than 2 be active at the same time.' },
      { icon: '🐉', name: 'Matter Scramble (Mogu Medium)', important: true, desc: 'Anima Golems spawn two ground markers. One player must stand in each - players swap locations harmlessly. If either marker has no player it explodes for ~500k to all. Call the far marker; someone near the melee covers the close one.' },
    ...
    heroic: [
      'Unstable Sparks: every completed crate sends one Spark (~1M HP, 10-second Supernova) to the other side.',
    ...
    lust: 'Second room, first boss crate - you just killed a lot of adds and have no cooldowns. Bloodlust fills the gap and burns the second boss fast.',
    diff: 4,
    roles: [
      ...
      { color: '#1db89a', label: 'Healer (per side)', note: 'Second crate opener - preemptively open the next set if tank needs help. Dispel Torment (Mogu Shadow Ritualist) on cooldown. Purge Rage of the Empress instantly. If you get Staff of Resonating Water, face mobs/spark and aim the wave proc at all times.' },
      { color: '#2e8fdf', label: 'DPS (3 per side)', note: 'Never open crates - that is the tank and healer\'s job. Kill priority: Burial Urns → Kunchong eggs → Stone Statues → Anima Golems → Amber Priests → Wind Wielders → Bombers. Use all DPS cooldowns on boss crates.' },
    ],
    strat: [
      {
        phase: 'Room 1 - Setup & Buff Crates',
        points: [
          { icon: null, role: 'All', note: 'Before dropping down, designate crate openers (tank + healer). Assign Matter Scramble marker roles: one player covers the near marker, one covers the far.' },
          { icon: '🛡️', role: 'Tank', note: 'Open both buff (green relic) crates first. The Brewmaster is the most dangerous spirit: face him away from the group and never let Breath of Fire cleave the raid. The Windwalker charges a random player - melee stands opposite to ranged so the fire trail doesn\'t cross the group.' },
          { icon: '💚', role: 'Healer', note: 'Once both spirits are dead, click the orb for your role. If you get the Staff of Resonating Water (Mistweaver kill), face toward mobs at all times - the wave proc is a massive damage and healing tool. Dispel the paralysis stun on any Windwalker charge target.' },
          { icon: '⚔️', role: 'DPS', note: 'Use personal cooldowns on the buff crates - no waste, you\'d fight these anyway. Brewmaster: stay behind. Windwalker: stand opposite from ranged. Mistweaver: move away during Gusting Crane Kick. Save major cooldowns for the boss crate.' },
        ],
      },
      {
        phase: 'Boss Crate - All Cooldowns',
        points: [
          { icon: null, role: 'All', note: 'Immediately after buff crates are cleared, the tank opens the boss crate plus 1–2 nearby small crates at the same time. Use all DPS cooldowns on the boss. Pull it toward the center so Unstable Sparks spawn in melee cleave range. Handle Stone Statues (Mogu) and Set to Blow bombs (Mantid) as they come.' },
          { icon: '🛡️', role: 'Tank', note: 'Mogu: kill statues as they spawn - boss AoE scales 20% per statue, never let more than 2 be active. Mantid: nothing special just tank the boss.' },
          { icon: '💚', role: 'Healer', note: 'Mogu: call for DPS if statues are falling behind - the pulsing AoE compounds fast, use CDs to survive. Mantid: heal through constant AoE and track Set to Blow players sprinting to drop bombs. Aim the Staff wave proc at the boss constantly.' },
          { icon: '⚔️', role: 'DPS', note: 'Full cooldowns on the boss. Mogu: DoT each new statue passively while tunneling the boss. Mantid: on Set to Blow, run to empty wall space, spam the Extra Action Button for all 4 stacks, then return.' },
        ],
      },
      {
        phase: 'Room Clear - Work Around the Room',
        points: [
          { icon: null, role: 'All', note: 'Once the boss is dead, keep 1–2 small crates open at all times while working around the room. Open mediums one at a time as you reach them - never two simultaneously. If a Burial Urn or Kunchong spawns, stop opening new crates and kill it before moving on. Both are stationary and will overwhelm the group if left up.' },
          { icon: '🛡️', role: 'Tank', note: 'Mogu: Announce Matter Scramble casts when golems are active - both markers must be covered. Move golems out of Crimson pools constantly. Stop opening mover when Burial Urns or Kunchong appear.' },
          { icon: '💚', role: 'Healer', note: 'Mogu: dispel Torment on cooldown - each dispel resets the damage and jumps it to a new target. Keep dispelling until the Ritualist is dead. Mantid: purge Rage of the Empress off Wind Wielders the instant it goes up. Aim the Staff wave proc at sparks to help kill them.' },
          { icon: '⚔️', role: 'DPS', note: 'Kill priority: Burial Urns and Kunchong eggs first - stop all other crate openings until they are dead. Cover Matter Scramble markers (Mogu) every time. Interrupt Wind Wielder casts and kill Amber Priests fast (Mantid). Bombers spawn persistent tornadoes - kill them before they fill the room.' },
        ],
      },
      {
        phase: 'Room 2 - Second Quadrant',
        points: [
          { icon: null, role: 'All', note: 'Both levers must be pulled before either group enters Room 2. Room 2 is the opposite mob type. Same flow: buff crates → boss crate with Bloodlust and all cooldowns (plus 1–2 smalls) → keep 1–2 smalls open while working the room → mediums one at a time → finish smalls to 50.' },
          { icon: '🛡️', role: 'Tank', note: 'Same role as Room 1. Announce boss crate opening as the Bloodlust trigger.' },
          { icon: '💚', role: 'Healer', note: 'Same crate-opening role. Remind the group of Bloodlust timing before entering. Continue all dispel and purge responsibilities.' },
          { icon: '⚔️', role: 'DPS', note: 'Bloodlust the moment the first boss crate mob is engaged in Room 2. Full cooldowns on the boss, then resume the same room-clear pattern.' },
        ],
      },
    ],
    abilities: [
